...
void solve(){
  ...
  vi freq(26);
  FORI(0, len) freq[s[i]-0x61]+=1;

  pair<pair<int, char>, int> lo, hi;

  lo = hi = {{freq[s[0]-0x61], s[0]}, 0};

  FORI(1, len){
    lo = min(lo, {{freq[s[i] - 0x61], s[i]}, i});
    hi = max(hi, {{freq[s[i] - 0x61], s[i]}, i});
  }
  s[lo.second] = s[hi.second];
  cout << s << "\n";
}
